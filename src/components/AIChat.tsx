  ...
  return (
    user && (
      <ExpandableChat size="md" position="bottom-right">
        ...
        <ExpandableChatBody>
          <ChatMessageList
            className="bg-muted/25 custom-scrollbar"
            ref={messagesRef}
          >
            {/* Initial message */}
                    ...
                    {message.content
                      ...
                      .map((part: string, index: number) => {
                        ...
                        const match = part.match(
                          /FileName:\s*(.+?)\?orgName=(\w+)/
                        );
                        if (match) {
                          const fileName = match[1]; // Extracted fileName
                          const orgName = match[2]; // Extracted orgName
                          const filePath = `/docs/${fileName}?orgName=${orgName}`;
                          return (
                            <span key={index}>
                              {/* Render the text before/after the filename intact */}
                              {part.replace(match[0], "")}
                              <Link
                                href={filePath}
                                className="text-blue-500 underline"
                              >
                                {fileName}?orgName={orgName}
                              </Link>
                            </span>
                          );
                        }
