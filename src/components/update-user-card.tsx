...
const UpdateUserCard = forwardRef<HTMLDivElement, UpdateUserCardProps>(
  ({ user, onClose }, ref) => {
    ...
    const handleUpdateUser = async () => {
      ...
      try {
        ...
        if (user.role !== role && role === "editor") {
          const usersOrgRef = doc(db, "org", user.org);
          const usersOrgDoc = await getDoc(usersOrgRef);
          await updateDoc(usersOrgDoc.ref, {
            editors: arrayUnion(user.id),
          });
        }
